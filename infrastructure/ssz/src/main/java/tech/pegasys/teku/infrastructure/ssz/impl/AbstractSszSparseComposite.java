 ...
 */
public abstract class AbstractSszSparseComposite<SszChildT extends SszData>
    implements SszSparseComposite<SszChildT> {

  private final IntCache<Optional<SszChildT>> childrenViewCache;
  private int sizeCache = -1;
  private final SszSparseCompositeSchema<?> schema;
  private final Supplier<TreeNode> backingNode;

  /** Creates an instance from a schema and a backing node */
  protected AbstractSszSparseComposite(
      final SszSparseCompositeSchema<?> schema, final Supplier<TreeNode> lazyBackingNode) {
    this(schema, lazyBackingNode, Optional.empty());
  }

  protected AbstractSszSparseComposite(final SszSparseCompositeSchema<?> schema, final TreeNode backingNode) {
   ...
   */
  protected AbstractSszSparseComposite(
      final SszSparseCompositeSchema<?> schema,
      final TreeNode backingNode,
      final IntCache<Optional<SszChildT>> cache) {
  ...
  protected AbstractSszSparseComposite(
      ...
      final Optional<IntCache<Optional<SszChildT>>> cache) {
    this.schema = schema;
   ...
   */
  IntCache<Optional<SszChildT>> transferCache() {
   ...
   * initial cache size
   */
  protected IntCache<Optional<SszChildT>> createCache() {
    return new ArrayIntCache<>();
  }

  @Override
  public final Optional<SszChildT> get(final int index) {
    return childrenViewCache.getInt(index, this::getImplWithIndexCheck);
  }

  private Optional<SszChildT> getImplWithIndexCheck(final int index) {
    checkIndex(index);
    return getImpl(index);
  }

  /** Cache miss fallback child getter. This is where child is created from the backing tree node */
  protected abstract Optional<SszChildT> getImpl(int index);

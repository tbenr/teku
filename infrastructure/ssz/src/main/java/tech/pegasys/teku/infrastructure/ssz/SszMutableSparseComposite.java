 ...
 */
public interface SszMutableSparseComposite<SszChildT extends SszData>
    extends SszMutableData, SszSparseComposite<SszChildT> {
   ...
   */
  void set(int index, Optional<SszChildT> value);

  /**
   * Similar to {@link #set(int, Optional)} but using modifier function which may consider old value
   * to calculate new value. The implementation may potentially optimize this case.
   */
  default void update(
      final int index, final Function<Optional<SszChildT>, Optional<SszChildT>> mutator) {
    set(index, mutator.apply(get(index)));
  }

  default void setAll(final Iterable<Optional<SszChildT>> newChildren) {
    clear();
    int idx = 0;
    for (Optional<SszChildT> newChild : newChildren) {
      set(idx++, newChild);
    }
  }

  @Override
  SszSparseComposite<SszChildT> commitChanges();
}

...
public interface SszStableContainerSchema<C extends SszStableContainer>
    extends SszSparseCompositeSchema<C> {
  ...
  static <C extends SszStableContainer> SszStableContainerSchema<C> create(
    ...
    return new AbstractSszStableContainerSchema<>(childrenSchemas) {
      @Override
  ...
  static <C extends SszStableContainer> SszStableContainerSchema<C> create(
      ...
      final BiFunction<SszStableContainerSchema<C>, TreeNode, C> instanceCtor) {
    return new AbstractSszStableContainerSchema<>(containerName, childrenSchemas) {
      @Override
      public C createFromBackingNode(TreeNode node) {
        return instanceCtor.apply(this, node);
      }
   ...
   */
  TreeNode createTreeFromFieldValues(List<Optional<? extends SszData>> fieldValues);
   ...
   * @throws IllegalArgumentException if value types doesn't match this scheme field types
   */
  default C createFromFieldValues(final List<Optional<? extends SszData>> fieldValues) {

...
class ForkChoiceModelGloas implements ForkChoiceModel {
  ...
  public boolean shouldBuildOnFull(
    ...
    checkArgument(
        ...
        "shouldBuildOnFull requires a non-pending head");
        ...
        .map(
            blockNodeVariants ->
                blockNodeVariants
                        .fullNode()
                        .flatMap(protoArray::getNode)
                        .filter(blockNode -> blockNode.getExecutionBlockHash().equals(blockHash))
                        .isPresent()
                    || protoArray
                        .getNode(blockNodeVariants.baseNode())
                        .filter(blockNode -> blockNode.getExecutionBlockHash().equals(blockHash))
                        .isPresent())
        .orElse(false);
  }
                    ...
                    .or(
                        () ->
                            protoArray
                                ...
                                .filter(
                                    blockNode ->
                                        blockNode.getExecutionBlockHash().equals(blockHash))));
  }
